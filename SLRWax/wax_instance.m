...
static int methodClosure(lua_State *L) {
    ...
    for (int i = 0; i < objcArgumentCount; i++) {
        ...
        if (type[0] == WAX_TYPE_STRING) {//因为在wax_copyToObjc方法中，没有对char*类型进行内存分配，所以这里不需要释放
            continue;
        }
        free(arguements[i]);
    }
...
static char * wax_charptr_call(id self, SEL _cmd, ...) {
    ...
    NSMethodSignature *signature = [self methodSignatureForSelector:_cmd];
    char *pReturnValue = wax_copyToObjc(L, [signature methodReturnType], -1, nil);//此方法中，对char*的处理，没有分配内存，所以下面不需要free
    END_STACK_MODIFY(L, 0)
    return pReturnValue;
}
#define WAX_METHOD_NAME_NEW(_type_, self, _cmd, invocation) wax_##_type_##_call_new(id self, SEL _cmd, NSInvocation *invocation)
...
static BOOL overrideMethod(lua_State *L, wax_instance_userdata *instanceUserdata) {
    ...
    SEL selector = foundSelectors[0];//如果找到了2个selector，则会使用第一个
    
    // instanceUserdata类的实现的class
    // 若原obj-C代码中无此lua类，则应该为空类。
    // 若原obj-C代码中有此lua类，则应该指向obj-C类。
    Class klass = [instanceUserdata->instance class];
    
    NSString *classStr = NSStringFromClass(klass);
    // Wax自带的方法替换，在这里屏蔽掉。
    // 如果不屏蔽，后面InterceptorView touchesEnded:Event:方法会使UIResponder的对应方法被替换，而引起异常
    // 屏蔽而引起的负面作用，目前还没有发现
    ...
    // 如果找到了，那么获得参数列表类型和返回值类型
    // 如果没找到，延父类继续向上寻找。
    if (method) { // Is method defined in the superclass?
        typeDescription = (char *)method_getTypeEncoding(method); // Returns a string describing a method's parameter and return types.
        returnType = method_copyReturnType(method); // Returns a string describing a method's return type.
    }
    ...
    ldaspect_block_t invocationBlock = ^(NSInvocation *invocation){
        ...
        switch (r_type[0]) {
            ...
            case WAX_TYPE_ID:
            {
                id returnValue = wax_id_call_new(invocation.target, invocation.selector, invocation);
                if (returnValue) {
            ...
            {
                int returnValue = wax_int_call_new(invocation.target, invocation.selector, invocation);
                [invocation setReturnValue:&returnValue];
            }
                break;
            ...
            {
                long returnValue = wax_long_call_new(invocation.target, invocation.selector, invocation);
                [invocation setReturnValue:&returnValue];
            }
                break;
            ...
            case WAX_TYPE_FLOAT:
            {
                float returnValue = wax_float_call_new(invocation.target, invocation.selector, invocation);
                [invocation setReturnValue:&returnValue];
            }
                break;
            ...
            {
                double returnValue = wax_double_call_new(invocation.target, invocation.selector, invocation);
                [invocation setReturnValue:&returnValue];
            }
                break;
            ...
            {
                BOOL returnValue = wax_BOOL_call_new(invocation.target, invocation.selector, invocation);
                [invocation setReturnValue:&returnValue];
            }
                break;
            case WAX_TYPE_STRING:
            {
                char *returnValue = wax_charptr_call_new(invocation.target, invocation.selector, invocation);
                [invocation setReturnValue:&returnValue];
            }
                break;
            case WAX_TYPE_STRUCT: {
                ...
                switch (size) {
                    case 16:
                    {
                        buffer_16 returnValue = wax_buffer_16_call_new(invocation.target, invocation.selector, invocation);
                        [invocation setReturnValue:&returnValue];
                    }
                        break;
                    case 32:
                    {
                        buffer_32 returnValue = wax_buffer_32_call_new(invocation.target, invocation.selector, invocation);
                        [invocation setReturnValue:&returnValue];
                    }
                        break;
                    default:
    ...
    if (returnType) { // Matching method found! Create an Obj-C method on the
        ...
        if(instImp) {
            ...
            NSDictionary *dict = @{@"class" : metaclass,
                                   @"sel" : selector ? NSStringFromSelector(selector) : [NSNull null],
                                   @"sel_objc" : newSelector ? NSStringFromSelector(newSelector) : [NSNull null], // objcXXXX
                                   @"typeDesc" : typeDescription ? [NSString stringWithUTF8String:typeDescription] : [NSNull null]
                                   };
		...
		for (int i = 0; i < 2; i++) {
			...
			while ((match = strchr(match, ':'))) { // match中首次出现:的位置
